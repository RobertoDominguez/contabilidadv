...
@section('content')
    ...
    <div class="card card-custom gutter-b example example-compact">
        ...
        <form action="{{ route('transaccion.update',$transaccion->id) }}" method="POST" class="form" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="card-body">
                @if ($errors->any())
                ...
                <div class="form-group">
                    ...
                    <input type="name" class="form-control form-control-solid" name="detalle"
                        placeholder="Introduce el detalle de la transaccion..." value="{{$transaccion->detalle}}" required />
                </div>
                <div class="form-group">
                    <label>Observaciones</label>
                    <input type="name" class="form-control form-control-solid" name="observaciones"
                        placeholder="Introduce la observacion si existe..." value="{{$transaccion->observaciones}}" " />
                </div>

                <div class="form-group">
                    <label>Total (Bs.)</label>
                    <input type="number" step="0.01" min="0" max="999" class="form-control form-control-solid" name="total"
                        placeholder="Introduce el total de la transaccion" value="{{$transaccion->total}}" required />
                </div>
